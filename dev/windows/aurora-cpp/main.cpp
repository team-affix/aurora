...
void tnn_multithread_test() {
	...
	vector<param_mom_mt*> pl = vector<param_mom_mt*>();
	...
	sync s = sync(pseudo::tnn({ 2, 20, 1 }, pseudo::nlr(0.3), [&](ptr<param>& pmt) {
		pmt = new param_mom_mt(urd(re), 0.002, 0, 0, 0.9);
		pl.push_back((param_mom_mt*)pmt.get());
	}));
