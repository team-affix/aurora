...
void lstm_ts::compile() {
	...
	m_tanh_gate->compile();
	m_forget_gate->m_x.group_link(m_gate_x);
	m_limit_gate->m_x.group_link(m_gate_x);
	m_input_gate->m_x.group_link(m_gate_x);
	m_output_gate->m_x.group_link(m_gate_x);
	m_tanh_gate->m_x.group_link(m_cty);
	m_y.group_link(m_hty);
	m_x_grad.group_link(m_htx_grad);
}
