...
void ntm_wh::compile() {
	...
	m_x.group_join_all_ranks(m_internal_rh->m_x);
	m_x.group_join_all_ranks(m_a_model->m_x);
	m_x.group_join_all_ranks(m_e_model->m_x);

	m_a.group_join_all_ranks(m_a_model->m_y);
	m_a_grad.group_join_all_ranks(m_a_model->m_y_grad);
	m_e.group_join_all_ranks(m_e_model->m_y);
	m_e_grad.group_join_all_ranks(m_e_model->m_y_grad);

	m_internal_rh->m_y.group_join_all_ranks(m_y);
	m_internal_rh->m_y_grad.group_join_all_ranks(m_y_grad);
}