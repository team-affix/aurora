...
void att_lstm::compile() {
	...
	this->m_y.group_link(m_internal_lstm->m_y);
	this->m_y_grad.group_link(m_internal_lstm->m_y_grad);
	for (int i = 0; i < l_a; i++) {
		att_lstm_ts* ats = (att_lstm_ts*)m_models->m_prepared[i].get();
		lstm_ts* lts = m_internal_lstm->m_prepared[i].get();
		this->m_x.group_link(ats->m_x);
		ats->m_htx.group_link(lts->m_htx);
		lts->m_x.group_link(ats->m_y);
		lts->m_x_grad.group_link(ats->m_y_grad);
	}
}
