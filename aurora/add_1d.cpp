...
void add_1d::compile()
{
	...
	{
		m_x.col(i).group_join_all_ranks(m_layer->m_x[i]);
		m_x_grad.col(i).group_join_all_ranks(m_layer->m_x_grad[i]);
		m_y[i].group_join_all_ranks(m_layer->m_y[i]);
		m_y_grad[i].group_join_all_ranks(m_layer->m_y_grad[i]);
	}
