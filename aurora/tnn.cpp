...
sequential* pseudo::tnn(vector<size_t> a_dims, Model a_neuron_template, function<void(ptr<param>&)> a_func) {
...
sequential* pseudo::tnn(vector<size_t> a_dims, vector<Model> a_neuron_templates, function<void(ptr<param>&)> a_func) {
	sequential* result = new sequential();
	for (int i = 0; i < a_dims.size() - 1; i++) {
...
sequential* pseudo::tnn_no_output(vector<size_t> a_dims, Model a_neuron_template, function<void(ptr<param>&)> a_func) {
	sequential* result = new sequential();
	for (int i = 0; i < a_dims.size() - 1; i++) {
		result->models.push_back(new layer(a_dims[i], a_neuron_template, a_func));
		result->models.push_back(new weight_junction(a_dims[i], a_dims[i + 1], a_func));
...
sequential* pseudo::tnn_no_output(vector<size_t> a_dims, vector<Model> a_neuron_templates, function<void(ptr<param>&)> a_func) {
	sequential* result = new sequential();
	for (int i = 0; i < a_dims.size() - 1; i++) {
		result->models.push_back(new layer(a_dims[i], a_neuron_templates[i], a_func));
		result->models.push_back(new weight_junction(a_dims[i], a_dims[i + 1], a_func));
