...
void ntm_rh::compile() {
	...
	m_key_model->m_x.group_link(m_x);
	m_beta_model->m_x.group_link(m_x);
	m_g_model->m_x.group_link(m_x);
	m_s_model->m_x.group_link(m_x);
	m_gamma_model->m_x.group_link(m_x);

	m_key.group_link(m_key_model->m_y);
	m_key_grad.group_link(m_key_model->m_y_grad);
	m_beta.group_link(m_beta_model->m_y);
	m_beta_grad.group_link(m_beta_model->m_y_grad);
	m_g.group_link(m_g_model->m_y);
	m_g_grad.group_link(m_g_model->m_y_grad);
	m_s.group_link(m_s_model->m_y);
	m_s_grad.group_link(m_s_model->m_y_grad);
	m_gamma.group_link(m_gamma_model->m_y);
	m_gamma_grad.group_link(m_gamma_model->m_y_grad);
