...
cnl::cnl(size_t a_filter_height, size_t a_filter_width, size_t a_stride_len, function<void(ptr<param>&)> a_init) {
...
cnl::cnl(size_t a_filter_height, size_t a_filter_width, size_t a_stride_len, ptr<model> a_filter_template) {
	this->filter_height = a_filter_height;
	this->filter_width = a_filter_width;
	this->stride_len = a_stride_len;
...
model* cnl::clone() {
	cnl* result = new cnl(filter_height, filter_width, stride_len, filter_template->clone());
	result->prep(prepared_x_height, prepared_x_width);
	result->unroll(unrolled_x_height, unrolled_x_width);
	return result;
}

model* cnl::clone(function<void(ptr<param>&)> a_init) {
	cnl* result = new cnl(filter_height, filter_width, stride_len, filter_template->clone(a_init));
	result->prep(prepared_x_height, prepared_x_width);
	result->unroll(unrolled_x_height, unrolled_x_width);
	return result;
}
...
void cnl::compile() {
	this->x = tensor::new_2d(prepared_x_height, prepared_x_width);
	this->x_grad = tensor::new_2d(prepared_x_height, prepared_x_width);
	this->y = tensor::new_2d(y_strides(prepared_x_height), x_strides(prepared_x_width));
	this->y_grad = tensor::new_2d(y_strides(prepared_x_height), x_strides(prepared_x_width));
	this->y_des = tensor::new_2d(y_strides(prepared_x_height), x_strides(prepared_x_width));
	this->y_des_reshaped = tensor::new_2d(y_strides(prepared_x_height), x_strides(prepared_x_width), { 0 });
	...
	filters->compile();
	for (int i = 0; i < y_strides(prepared_x_height); i++) {
		int y_start = i * stride_len;
		sync* row = (sync*)filters->prepared[i].get();
		for (int j = 0; j < x_strides(prepared_x_width); j++) {
...
void cnl::prep(size_t a_a, size_t a_b) {
	this->prepared_x_height = a_a;
	this->prepared_x_width = a_b;
	filters->unrolled.clear();
	filters->prepared.clear();
...
void cnl::unroll(size_t a_a, size_t a_b) {
	this->unrolled_x_height = a_a;
	this->unrolled_x_width = a_b;
...
size_t cnl::x_strides() {
	return (prepared_x_width - filter_width) / stride_len + 1;
}
...
size_t cnl::y_strides() {
	return (prepared_x_height - filter_height) / stride_len + 1;
}