...
namespace aurora {
	namespace pseudo {
		...
		sequential* tnn_no_output(vector<size_t> a_dims, ptr<model> a_neuron_template, function<void(ptr<param>&)> a_func);
		sequential* tnn_no_output(vector<size_t> a_dims, vector<ptr<model>> a_neuron_templates, function<void(ptr<param>&)> a_func);
	}
}